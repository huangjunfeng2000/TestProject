...
using namespace std;
CManageDB *CManageDB::s_pManager = NULL;
//////////////
CManageDB::CManageDB()
{
	m_bActive = false;
...
CManageDB *CManageDB::instance()
{
	if (!s_pManager)
		s_pManager = new CManageDB();
	return s_pManager;
}
void CManageDB::ReleaseInstance()
{
	if (s_pManager)
	{
		delete s_pManager;
...
bool CManageDB::ConnectDB(const std::string &strDB)
{
		try
		{
			db.open(strDB.c_str());
...
bool CManageDB::BeginTrans()
{
	return ExecCommand("begin transaction;");
}
bool CManageDB::CommitTrans()
{
	return ExecCommand("commit transaction;");
}
bool CManageDB::RollbackTrans()
{
...
//	return strResult;
//}
//bool CManagePower::ProcessCommand(const char * strCmd, size_t iSize)
//{
//	int iType;
...
//	}
//}
CppSQLite3Query CManageDB::ExecQuery(const std::string &strSql)
{
...
bool CManageDB::ExecCommand(const std::string &strSql)
{
	//sbuf.clear();
	//msgpack::packer<msgpack::sbuffer> packer(sbuf);
...
bool CManageDB::ExecCommandBin(const std::string &strSql, const unsigned char *pStr, size_t iSize)
{
	//sbuf.clear();
	//msgpack::packer<msgpack::sbuffer> packer(sbuf);
